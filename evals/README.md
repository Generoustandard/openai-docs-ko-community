...
- [build_goldens.py](/C:/Users/user/Documents/Repo/openai-docs-ko-community/evals/build_goldens.py)
  `docs/inputs/urls.txt`의 문서 URL을 읽어 `.md` 스냅샷과 `docs/golden/*.json` 파일을 생성합니다. 한국어 `golden` 초안은 OpenAI 모델이 자동으로 만듭니다.
...
## build_goldens.py 사용법

먼저 `.env`에 `OPENAI_API_KEY`를 넣고 `docs/inputs/urls.txt`에 대상 URL을 적습니다.

```bash
python evals/build_goldens.py
```

기본값으로 다음 동작을 수행합니다.

- URL 본문을 수집해 `docs/inputs/md/*.md`에 저장
- 원문을 `words`, `sentences`, `paragraphs` 단위로 분리
- `bad_ko`, `improved_ko`, `notes`, `tags`를 OpenAI 모델로 자동 생성
- 결과를 `docs/golden/words.json`, `docs/golden/sentences.json`, `docs/golden/paragraphs.json`에 저장

모델을 바꾸고 싶다면 다음처럼 실행할 수 있습니다.

```bash
python evals/build_goldens.py --model gpt-5.4
```
